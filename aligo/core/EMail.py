...
def send_email(recevier: str, title: str, content: str, qr_data: bytes):
    """发送邮件"""
    sender = 'aligo_notify@163.com'

    msg_root = MIMEMultipart()
    msg_root['From'] = formataddr(('aligo notify', sender))
    msg_root['To'] = formataddr((recevier, recevier))
    ...
    for i in range(1, 4):
        try:
            ...
            time.sleep(i * 3)
    assert False, f'邮件发送失败 {recevier}'
